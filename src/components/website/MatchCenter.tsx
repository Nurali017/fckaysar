...
const CountdownItem = memo(({ value, label }: { value: number; label: string }) => (
    <div className="flex flex-col items-center bg-zinc-900/80 border border-white/5 rounded-xl sm:rounded-2xl p-2 sm:p-4 md:p-6 backdrop-blur-sm group hover:border-red-500/50 transition-all duration-300 hover:shadow-[0_0_20px_-5px_rgba(220,38,38,0.2)]">
        ...
        <span className="text-[8px] sm:text-[10px] md:text-xs uppercase tracking-widest text-zinc-500 font-bold group-hover:text-zinc-300 transition-colors">{label}</span>
    </div>
));
CountdownItem.displayName = 'CountdownItem';

// Memoized stat bar to prevent re-renders
const StatBar = memo(({ label, leftValue, rightValue }: { label: string; leftValue: number; rightValue: number }) => (
    <div className="space-y-1 sm:space-y-2">
        <div className="flex justify-between text-xs sm:text-sm font-bold">
            <span>{leftValue}%</span>
            <span className="text-gray-400 uppercase text-[10px] sm:text-xs tracking-widest">{label}</span>
...
export const MatchCenter = () => {
    ...
    const currentYear = currentSeason
        ? new Date(currentSeason.endDate).getFullYear()
        : 2025;
    ...
    if (!nextMatch) {
        ...
        return (
            <section className="container mx-auto px-4 py-12 sm:py-16 md:py-20 relative overflow-hidden">
                {/* Background Elements */}
                <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-black/50 to-transparent pointer-events-none" />
                <div className="absolute -top-40 -right-40 w-64 sm:w-96 h-64 sm:h-96 bg-red-600/20 rounded-full blur-3xl" />
                <div className="absolute -bottom-40 -left-40 w-64 sm:w-96 h-64 sm:h-96 bg-blue-600/10 rounded-full blur-3xl" />
                ...
                <div className="relative z-10 max-w-5xl mx-auto">
                    <div className="text-center mb-8 sm:mb-12">
                        ...
                        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 sm:mb-6 uppercase italic tracking-tighter">
                            {t("matches.seasonSummary")} <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-red-800">{currentYear}</span>
                        </h2>
                    </div>
                            ...
                            <div className="space-y-4 sm:space-y-6 md:space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                                    <div className="bg-zinc-900/80 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-white/5 hover:border-red-500/50 transition-all duration-300 hover:shadow-[0_0_30px_-10px_rgba(220,38,38,0.3)] group/stat">
                                        <div className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-1 group-hover/stat:text-red-500 transition-colors">{teamStats?.raw.points || '--'}</div>
                                        <div className="text-[10px] sm:text-xs text-zinc-500 font-bold uppercase tracking-widest group-hover/stat:text-zinc-300">{t("stats.labels.points")}</div>
                                    </div>
                                    <div className="bg-zinc-900/80 p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-white/5 hover:border-red-500/50 transition-all duration-300 hover:shadow-[0_0_30px_-10px_rgba(220,38,38,0.3)] group/stat">
                                        <div className="text-3xl sm:text-4xl md:text-5xl font-black text-white mb-1 group-hover/stat:text-red-500 transition-colors">{teamStats?.raw.games_played || 0}</div>
                                        <div className="text-[10px] sm:text-xs text-zinc-500 font-bold uppercase tracking-widest group-hover/stat:text-zinc-300">{t("matches.games")}</div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-center text-xs sm:text-sm px-1 sm:px-2 py-2 sm:py-3 border-t border-b border-white/5 text-zinc-400">
                                    <span className="flex flex-col md:flex-row gap-1 items-center">
                                        ...
                                        <span className="text-white font-bold text-base sm:text-lg">{teamStats?.raw.win || 0}</span>
                                    </span>
                                    <span className="w-px h-6 sm:h-8 bg-white/10 mx-1 sm:mx-2" />
                                    <span className="flex flex-col md:flex-row gap-1 items-center">
                                        <span>{t("matches.draws")}:</span>
                                        <span className="text-white font-bold text-base sm:text-lg">{teamStats?.raw.draw || 0}</span>
                                    </span>
                                    <span className="w-px h-6 sm:h-8 bg-white/10 mx-1 sm:mx-2" />
                                    <span className="flex flex-col md:flex-row gap-1 items-center">
                                        <span>{t("matches.losses")}:</span>
                                        <span className="text-white font-bold text-base sm:text-lg">{teamStats?.raw.match_loss || 0}</span>
                                    </span>
                                </div>
        ...
        <section className="container mx-auto px-4 py-12 sm:py-16 md:py-20 relative overflow-hidden">
            ...
            <div className="relative z-10">
                <div className="text-center mb-8 sm:mb-12">
                    ...
                    <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 sm:mb-6 uppercase italic tracking-tighter">
                        Next <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-red-800">Battle</span>
                    </h2>
                </div>
                                ...
                                >
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-between w-full gap-4 sm:gap-6 md:gap-8 mb-8 sm:mb-12">
                                        <div className="flex flex-col items-center gap-2 sm:gap-4 flex-1">
                                            <img src={nextMatch.homeTeam.logo} alt={nextMatch.homeTeam.name} className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 object-contain drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]" />
                                            <span className="text-base sm:text-lg md:text-xl lg:text-2xl font-black uppercase tracking-tighter text-center">{nextMatch.homeTeam.name}</span>
                                        </div>

                                        <div className="flex flex-col items-center gap-1 sm:gap-2">
                                            <span className="text-2xl sm:text-3xl md:text-4xl font-black text-white/20">VS</span>
                                        ...
                                        <div className="flex flex-col items-center gap-2 sm:gap-4 flex-1">
                                            <img src={nextMatch.awayTeam.logo} alt={nextMatch.awayTeam.name} className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-32 lg:h-32 object-contain drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]" />
                                            <span className="text-base sm:text-lg md:text-xl lg:text-2xl font-black uppercase tracking-tighter text-center">{nextMatch.awayTeam.name}</span>
                                        </div>
                                    </div>
