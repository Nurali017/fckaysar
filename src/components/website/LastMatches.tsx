...
export const LastMatches = () => {
      ...
      <section className="container mx-auto px-4 py-12">
        ...
        <div className="flex gap-4">
          {[1, 2, 3, 4, 5].map(i => (
            <Skeleton
              key={i}
              className="w-[min(288px,85vw)] md:w-80 h-48 bg-white/10 rounded-2xl"
            />
          ))}
        </div>
      </section>
  ...
  if (lastMatches.length === 0) {
    return (
      <section className="container mx-auto px-4 py-12">
        <div className="text-center text-gray-400">No recent results available.</div>
      </section>
    );
  }
  ...
  return (
    <section className="container mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <Link
          to="/matches"
          className="text-sm text-gray-400 hover:text-white transition-colors font-bold uppercase tracking-wider flex items-center gap-1"
        >
      ...
      <ScrollArea className="w-full whitespace-nowrap pb-4">
        <div className="flex gap-4">
          {lastMatches.map((match, index) => (
            <MatchResultCard key={match.id} match={match} index={index} isMobile={isMobile} />
          ))}
        </div>
  ...
  return (
    <Link to={`/match/${match.id}`}>
      ...
      >
        ...
        <div
          className={`absolute top-0 left-0 w-1 h-full ${match.isWin ? 'bg-green-500' : match.score.includes(match.score.split(' : ')[0] === match.score.split(' : ')[1] ? 'bg-gray-500' : 'bg-red-500')}`}
        />
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-col items-center">
            <span
              className={`text-3xl font-black tracking-tighter ${match.isWin ? 'text-green-500' : 'text-white'}`}
            >
              {match.score}
            </span>
